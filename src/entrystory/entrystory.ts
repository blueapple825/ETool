class user{
    ...
    constructor(id:string,username:string,nickname:string,profileImage:string,backgroundImage:string,description:string) {
        this.id=id;
...
function uploadImage(file:File):void{ //이미지 업로드 함수
...
function getSticker(src:string):string{
    switch(src){
...
function uploadPost(){
    var content:string|undefined=document.getElementById("Write")?.innerHTML;
    var stickerID:string="null";
    try{
        var stickerHTML:string=document.getElementsByClassName("e1h77j9v3")[0].innerHTML;
        var stickerID:string=stickerID=getSticker(stickerHTML.substr(stickerHTML.indexOf("src=\"")+5,51));
    }catch(e){}
    if(content!=""){
        (async function(){
...
const stopEvent=function(e:Event):boolean{e.preventDefault();e.stopPropagation();return false;}; //이벤트 삭제에 사용할 함수
...
async function getUserData(index:number){
    ...
    let ids:string|undefined=document.getElementsByClassName("enx4swp0")[index+1].getAttribute("href")?.substr(9);
    if(ids==undefined){
    ...
    var id:string=data.data.userstatus.id;
    var username:string=data.data.userstatus.username;
    var nickname:string=data.data.userstatus.nickname;
    var profileImgage:string;
    try{
    ...
    var description:string=data.data.userstatus.description;
    var u:user=new user(id,username,nickname,profileImgage,backgroundImage,description);
    userData[index]=u;
}
...
function showUserData(index:number){
    ...
    var userDataBGImg:any=Array.from(document.getElementsByClassName("userDataBGImg") as HTMLCollectionOf<HTMLElement>)[0];
    userDataBGImg.style.backgroundImage="url(\""+userData[index].backgroundImage+"\")";

    var userDataPFImg:any=Array.from(document.getElementsByClassName("userDataPFImg") as HTMLCollectionOf<HTMLElement>)[0];
    userDataPFImg.style.backgroundImage="url(\""+userData[index].profileImage+"\")";

    var userDataID:any=Array.from(document.getElementsByClassName("userDataID") as HTMLCollectionOf<HTMLElement>)[0];
    userDataID.innerText=userData[index].id;

    var userDataName:any=Array.from(document.getElementsByClassName("userDataName") as HTMLCollectionOf<HTMLElement>)[0];
    userDataName.innerText=userData[index].nickname+"("+userData[index].username+")";

    var userDataD:any=Array.from(document.getElementsByClassName("userDataD") as HTMLCollectionOf<HTMLElement>)[0];
    userDataD.innerText=userData[index].description;
}

function getUsersData(index:number,size:number):void{
    try{
